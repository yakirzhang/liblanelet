...
class distance_heuristic : public boost::astar_heuristic<G, double> {
 public:
  ...
  double operator()(typename G::vertex_descriptor u) { return 0; }
...
struct Dijkstra {
  ...
  Dijkstra(G &graph, vertex_t source) : graph(graph), source(source) {
    ...
    boost::dijkstra_shortest_paths(graph, source, predecessor_map(_predecessors.data())
                                                      .distance_map(_distances.data())
  ...
  std::deque<vertex_t> shortest_path(vertex_t target) {
    ...
    if (!reached(target)) return EMPTY_LIST;
    ...
    assert(sp.empty() ||
           (sp.front() == source && sp.back() == target) &&
               "Dijkstra returns either empty list or path from source to "
               "target.");
...
template <typename G>
boost::optional<std::vector<typename G::vertex_descriptor>> dijkstra_shortest_path(G &g,
                                                                                   typename G::vertex_descriptor from,
                                                                                   typename G::vertex_descriptor to) {
  boost::optional<std::vector<typename G::vertex_descriptor>> result;
  auto dij = Dijkstra<G>(g, from);

  if (!dij.reached(to)) return result;
  ...
  assert((my_sp.empty() && sp.empty()) ||
         (my_sp.front() == sp.front() && my_sp.back() == sp.back()) && "deque and vector are equal.");
