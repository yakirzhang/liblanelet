...
struct BoundingBox {
  ...
  template <int index> const double &get() const { return std::get<index>(bb); }

  BoundingBox(const point_with_id_t &pt)
      : bb(std::make_tuple(std::get<LAT>(pt), std::get<LON>(pt),
                           std::get<LAT>(pt), std::get<LON>(pt))) {}
  ...
  void extend_box(const BoundingBox &other) {
    std::get<NORTH>(bb) =
        std::max(std::get<NORTH>(bb), std::get<NORTH>(other.bb));
    std::get<SOUTH>(bb) =
        std::min(std::get<SOUTH>(bb), std::get<SOUTH>(other.bb));
